...
data<-read.table("Mass-independent MMRdatab1.txt", header=T)
...
data<-read.table("Mass-independent MMRdata1.txt", header=T) # Reload data to plot it with log10 conversion
...
plot(NA,ylim=c(1L,length(tree2$tip.label)),xlim=lab,axes=FALSE,xlab="MMR (mLO2 h-1 g-0.67)")
lab <- lab[1L]:lab[2L]
